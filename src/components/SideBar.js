...
const fetchTollCost = async (fromCoords, toCoords, vehicleType) => {
  ...
  return data?.response?.route[0]?.cost || null; // Returns toll cost details
};

const SideBar = ({ onPolylineFetched }) => {
  ...
  const handleFetchRouteDetails = async () => {
    ...
    if (fromCoordinates && toCoordinates) {
      try {
        ...
        if (polyline) {
          console.log("Fetched Polyline:", polyline);
        ...
        main={
          <HDSVerticalNavigation>
            <div className="side-panel-content">
              ...
              />
              <br />
              {/* Fetch and Display Route and Toll Costs */}
           ...
           variant="success">
              ...
              :
              <>
              <p>Estimated value is : 
              {tollCost.totalCost ? tollCost.totalCost +" "+ tollCost.currency: " "}
              </p>
              </>
              }
